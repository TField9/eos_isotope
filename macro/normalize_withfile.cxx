...
void normalize_withfile(){

    TFile *normalize_para = new TFile("/eos/ams/user/s/selu/mdst/tianye/root/normalize_parameter/normalize_para.root","read");
    TFile *f1 = new TFile("/eos/ams/user/s/selu/mdst/tianye/root/normalize_parameter/para_store.root","recreate");
    TH2D *h_be7[3][7];
    TH2D *h_be9[3][7];
    TH2D *h_be10[3][7];
    double fit_range_fac=2;
    ...
    for(int i=1;i<8;i++){
        for(int j=0;j<3;j++){
            ...
            h_be7[j][i-1]= (TH2D*)normalize_para->Get(Form("be7_%d_%d",j,i));
            h_be9[j][i-1]= (TH2D*)normalize_para->Get(Form("be9_%d_%d",j,i));
            h_be10[j][i-1]= (TH2D*)normalize_para->Get(Form("be10_%d_%d",j,i));
        }
    }
    ...
    for(int i=1;i<8;i++){
        for(int j=0;j<3;j++){
            int rebin_factor;
            if(j==0) rebin_factor = 2;
            else rebin_factor = 5;

            h_be7[j][i-1]->RebinY(rebin_factor);
            h_be9[j][i-1]->RebinY(rebin_factor);
            h_be10[j][i-1]->RebinY(rebin_factor);
        }
    }
            ...
            if (py->GetEntries() > 10) {
                ...
                if (f) {
                    ...
                    if (std::isfinite(sigma) && sigma > 0) {
                        double xmin = mean - fit_range_fac * sigma;
                        double xmax = mean + fit_range_fac * sigma;
                        // refined fit around ±2 sigma
                        py->Fit("gaus", "RQ", "", xmin, xmax);
    ...
    // Save mean and sigma histograms to file
