...
# # 将 ML 环境的 bin 目录添加到 PATH
# export PATH="/work/miniconda/envs/ML/bin:$PATH"

# # 然后直接调用 python
# python /eos/ams/user/s/selu/mdst/tianye/transformer/stdcut.py "$input" "$output"


# Execute the sdst binary with output filename

export PATH="/afs/cern.ch/work/s/seludev/private/tianye/miniconda/ML/bin:$PATH"
eos cp /eos/ams/user/s/selu/mdst/tianye/sh/param_std2.txt ./
eos cp /eos/ams/user/s/selu/mdst/tianye/transformer/stdcut.py ./
chmod +x stdcut.py
python ./stdcut.py $input $output

sleep 1
sync 
sleep 1
odir="/eos/ams/user/s/selu/mdst/tianye/root/Be_MC_ML_train"
echo to cp ${output} ${odir}

xrdcp --retry 3 ${output} root://eosams.cern.ch/${odir}/${output}
